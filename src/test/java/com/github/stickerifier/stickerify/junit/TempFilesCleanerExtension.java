 ...
 */
public class TempFilesCleanerExtension implements AfterAllCallback {
	...
	private static void deleteTempFiles() throws IOException {
		try (var files = Files.list(Path.of(System.getProperty("java.io.tmpdir")))) {
			files.filter(Files::isRegularFile)
					.map(Path::toFile)
					.filter(TempFilesCleanerExtension::stickerifyFiles)
					.forEach(TempFilesCleanerExtension::deleteFile);
		}
	}
